...
class ParentQuiz(models.Model):
    ...
    def get_num_of_question(self):
        # each question field should have at least one parent field
        all_num_list = []
...
class Question(models.Model):
    ...
    text = models.CharField(max_length=400, default='', null=True)
...
class QuizTaker(models.Model):
    user = models.ForeignKey(User, null=False, related_name='quiz_taker', on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.max_grade==None:
            ...
            if str(grade[self.grade.name]) > str(grade[self.max_grade]):
                self.max_grade = self.grade.name
                self.max_level = self.level
            elif grade[self.grade.name] == grade[self.max_grade] and str(self.level) > str(self.max_level):
                self.max_level = self.level
...
def gogleInitialization(sender, instance, created, **kwargs):
    if created:
        print("RECeiVER")
