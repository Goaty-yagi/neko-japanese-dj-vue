...
class UserStatusSerializer(serializers.ModelSerializer):
	...
	def create(self, validated_data):
		...
		if UserStatus.objects.filter(**validated_data).exists():
			print('exist', UserStatus.objects.filter(**validated_data))
			if is_correct:
			...
			if is_correct:
				...
				user_status = UserStatus.objects.create(
					**validated_data,
					is_false=is_false)
		print('lenseri',len(connection.queries),user_status)
		return user_status
	...
	def create(self, validated_data):
		...
		if MyQuestion.objects.filter(my_quiz=my_quiz,question__id=question.id).exists():
			...
			if int(num.values_list('my_question__count')[0][0]) >= my_quiz.max_num:
				...
				my_question = MyQuestion.objects.create(my_quiz=my_quiz,question_id=question.id)
				print('created',my_question)
				return  my_question
