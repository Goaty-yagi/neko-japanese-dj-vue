...
def pick_random_object(queryset,quiz_num):
    """receive num and queryset which have specific range.
...
class QuestionImageDispatchApi(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        serializer.save(image=thumbnail)
...
class QuizTakerTestPatchApi(APIView):
    def patch(self, request):
        ...
        print('quiz_TAKER', quiz_taker)
        quiz_taker.save()
        # time = dateutil.parser.isoparse(time)
        # print("time", time)
        user = User.objects.get(quiz_taker__id=quiz_taker_id)
        user.test_taken = True
        # user.test_taken_at = time
        user.save()
...
class QuizTakerPracticePatchApi(APIView):
    def patch(self, request):
        quiz_taker_id = request.query_params['quiz_taker']
        quiz_taker_obj = QuizTaker.objects.filter(id=quiz_taker_id)
        quiz_taker_obj.update(
            practice_take_num=F('practice_take_num') + 1,
        )
        seri = QuizTakerStorageSerializer(quiz_taker_obj[0])
        return Response(seri.data)

class QuizTakerCreateApi(APIView):
    def post(self, request):
        print("QTC")
        user = request.user
        print(user)
        if user:
            QuizTaker.objects.create(user=user, **request.data)
            return Response({'message':"created"})
        else:
            return Response({"message":'fail creation'}, status=400)
...
class QuizApi(APIView):
    def get(self, request):
        ...
        if request.query_params.getlist("field"):
            f = [i.strip("[]") for i in request.query_params.getlist("field")if i != ',']
            i = ''.join(f[0])
            field_list = i.split(',')
...
# this is for getting question from my question
    def get(self, request, format=None):
        f = [i.strip("[]") for i in request.query_params.getlist("ids")if i != ',']
        i = ''.join(f[0])
        id_list = i.split(',')
...
class QuizTakerDetailApi(generics.RetrieveUpdateDestroyAPIView):
    queryset = QuizTaker.objects.select_related(
        'user',
        'grade'
    ).prefetch_related(
...
class MyQuizDetailApi(generics.RetrieveUpdateDestroyAPIView):
    queryset = MyQuiz.objects.prefetch_related(
        'my_question',
        'my_question__question',
        'my_question__question__field',
        'my_question__question__status',
    ).select_related('user')
    serializer_class = MyQuizSerializer
    pagination_class = None
    lookup_field = 'user'
...
class QuizTakerUpdate(APIView):
    "this is for initial user setup purpose"
    def post(self, request, format=None):
        "recive quiz_taker_id, statusList, grade , level. status_list is for UserStatus"
        user_UID = request.data['user_UID']
        print("QTUPDATE")
       
        try:
            user_status = request.data["quiz_taker"]["statusList"]
            grade = request.data["quiz_taker"]["grade"]
            level = request.data["quiz_taker"]["level"]
            quiz_taker_object = QuizTaker.objects.get(user=user_UID)
            quiz_taker_object.level = level
            quiz_taker_object.test_take_num = 1
            quiz_taker_object.save()
            for i in user_status:
                parent_status = ParentStatus.objects.get(id=i['status'])
                user_status = UserStatus.objects.get(quiz_taker=quiz_taker_object, status=parent_status)
                user_status.is_correct=i['isCorrect']
                user_status.is_false=i['isFalse']
                user_status.save()
            serializer =  QuizTakerStorageSerializer(data=request.data["quiz_taker"])
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=201)
            return JsonResponse({"updated":True})
        except:
            print("EXCEPT", quiz_taker_object)
            JsonResponse({"updated":False})
# class UserInitializationApi(APIView):
#     def patch(self, request):
#         print("patch")
#         try:
#             UID = request.data["UID"]
#             quiz_taker = QuizTaker.objects.get(user__UID=UID)
#             quiz_taker.level = request.data["quiz_taker"][2]
#             user_status = request.data["quiz_taker"][2]
#             if serializer.is_valid():
#                 serializer.save()
#                 print('saveed')
